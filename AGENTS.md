...
- **Interval & window**: The scheduler processes 60-minute windows aligned to the top of the hour (xx:00). The cron entry should be `0 * * * * /Users/<username>/src/github.com/wakamenori/activitywatch-util/scripts/run-range-analysis-cron.sh`.
- **Setup**: Install the cron job with `(crontab -l 2>/dev/null; echo "0 * * * * …/scripts/run-range-analysis-cron.sh") | crontab -`. Cron’s minimal environment is handled inside the script; no LaunchAgent files are required. Ensure the wrapper keeps its execute bit (`chmod +x scripts/run-range-analysis-cron.sh`) or cron will fail with `Permission denied`.
