...
export function ActivitySunburst({ timeRange }: { timeRange: string }) {
	...
	const chartRoot = useMemo(() => {
		...
		const build = (
			...
			const nextCat =
				depth === 0 ? undefined : depth === 1 ? node.name : catName;
			const nextApp = depth === 2 ? node.name : appName;
			const base = nextCat
				? CATEGORY_COLORS[nextCat as keyof typeof CATEGORY_COLORS] ||
			...
			const nextLeafSeq =
				depth === 1 ? { i: 0, total: countLeaves(node) } : leafSeq; // per-category sequence
		...
		return data?.root
			? build(data.root, [], 0, undefined, undefined, 0, 0, undefined)
			: null;
	}, [data]);
				...
				<div className="relative h-[360px] md:col-span-4">
					...
					<ResponsiveSunburst
						...
						tooltip={(args: unknown) => {
							...
							return (
								<div
									className="rounded-md px-2 py-1 text-sm shadow bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600"
									style={{ color: "inherit" }}
								>
									...
									<div className="text-gray-600 dark:text-gray-300">
										{formatTotal(n.value || 0)}（{Math.round(n.percentage || 0)}
										%）
									</div>
